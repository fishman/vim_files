...
if ! exists ('g:visual_studio_quickfix_errorformat_cpp')
    let g:visual_studio_quickfix_errorformat_cpp  = 
        ...
        \'%f(%l)\ :\ %m,'.
        \'\ %#%f(%l)\ :\ %m'
    " we could include these if we want
...
function! DTEListFiles(...)
    ...
    "  a:1 -- name of the project to fetch files for
    ...
    if exists("project_name")
        call <Sid>DTEExec ('dte_list_files', a:1)
    else
        call <Sid>DTEExec ('dte_list_files')
    endif

    for s:file in s:visual_studio_lst_project_files
        echomsg s:file
    endfor
endfunction
...
function! DTEGetFiles(...)
    " Optional args passed in are
    "  a:1 -- name of the project to fetch files for
    "  a:2 -- if true then give feedback re projects
    let verbose = 1
    if a:0 >= 1 | let project_name = a:1 | endif
    if a:0 >= 2 | let verbose = a:2 | endif
    ...
    if exists("project_name")
        call <Sid>DTEExec ('dte_list_files', project_name)
    else
        call <Sid>DTEExec ('dte_list_files')
    endif
    ...
    if verbose
        echo 'Found '.len(s:visual_studio_lst_project_files).' file(s)'
    endif

    exe "args " . join(s:visual_studio_lst_project_files, ' ')
endfunction
    ...
    if g:visual_studio_quickfix_height > 0
        ...
        if &buftype == 'quickfix'
            if g:visual_studio_quickfix_path != ''
                exe 'silent! lcd '.g:visual_studio_quickfix_path
            endif
        endif
    endif
...
function! DTEBuildProject(...)
    let &errorfile = g:visual_studio_output
    if a:0 > 0
        call <Sid>DTEExec ('dte_build_project', &errorfile, g:visual_studio_write_before_build, a:1)
    else
...
if ! exists ('g:visual_studio_mapping') || g:visual_studio_mapping != 0
    ...
    nmap <silent> <Leader>vu :call DTEBuildProject()<cr>
