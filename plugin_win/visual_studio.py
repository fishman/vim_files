...
def dte_build_project(vs_pid, fn_quickfix, write_first, project_name=None):
    ...
    try:
        ...
        project_unique_name = _get_project(dte, project_name or
                dte.Solution.Properties("StartupProject").Value).UniqueName
        logging.info ('SolutionBuild.BuildProject configuration_name: %s, ' +
                'project_unique_name: %s' %
...
##def dte_echo_files(vs_pid, project_name=None):
##    logging.info('== dte_list_files %s' % vars())
##    for f in dte_get_files(vs_pid, project_name):
##        _vim_echo(f)

def dte_list_files(vs_pid, project_name=None):
...
def _dte_get_proj_files(vs_pid, project_name=None):
    ...
    files = []
    try:
        project = _get_project(dte, project_name or
